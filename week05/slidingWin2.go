...
func (slideWindow *SlideWindow) Print() {
    log.Println(slideWindow)
    // 打印节点
    chain := slideWindow.BucketChain
    for i := 0; i < slideWindow.BucketCount; i++ {
        log.Println(chain.Value)
        chain = chain.Next()
    }
}

func main() {
    slideWindow, err := Init(1, 5, 10)
    if err != nil {
        log.Printf("Error: %s \n", err)
        return
    }
    ...
    go func() {
        ...
        for i := 0; i < 200; i++ {
            ...
            if err != nil {
                log.Printf("Request a%d\t\t %s \n", i, err)
            } else {
                log.Printf("Request a%d\t\t passed \n", i)
            }
    ...
    go func() {
        ...
        for i := 0; i < 200; i++ {
            err := slideWindow.Limiting()
            if err != nil {
                log.Printf("Request b%d\t\t %s \n", i, err)
            } else {
                log.Printf("Request b%d\t\t passed \n", i)
            }
